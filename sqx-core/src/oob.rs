...
impl OobServer {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *self.http_handle.write().await = Some(http_handle);
        ...
        let dns_public_host = self.config.public_host.clone();
        let dns_handle = tokio::spawn(async move {
            if let Err(e) = run_dns_server(dns_port, dns_domain, dns_public_host, dns_interactions, dns_ids, dns_shutdown_rx).await {
...
/// Run DNS server using async tokio UDP socket.
async fn run_dns_server(
    ...
    // Resolve public_host to an IPv4 address; fall back to 127.0.0.1 on failure.
    let response_ip: [u8; 4] = match public_host.parse::<std::net::Ipv4Addr>() {
        Ok(ip) => ip.octets(),
        Err(_) => {
            // Try DNS resolution
            match tokio::net::lookup_host(format!("{}:80", public_host)).await {
                Ok(mut addrs) => {
                    if let Some(addr) = addrs.find(|a| a.is_ipv4()) {
                        let octets = addr.ip().to_string()
                ...
                Err(e) => {
                    warn!("Failed to resolve public_host '{}': {}, using 127.0.0.1", public_host, e);
                    [127, 0, 0, 1]
                }
            }
        }
    ...
    info!(
        "DNS OOB server listening on {} (responses will point to {})",
        addr,
        std::net::Ipv4Addr::from(response_ip)
    );
                ...
                Ok((len, src)) => {
                    ...
                    if let Ok(query) = parse_dns_query(&buf[..len])
                        && query.ends_with(&domain_clone) {
                            ...
                            let response = build_dns_response(&buf[..len], &response_ip);

...
use super::tamper::{
    TamperScript,
    // original 30
    TamperSpaceToComment, TamperSpaceToTab, TamperSpaceToNewline, TamperSpaceToWhitespaceMix,
    TamperUrlEncode, TamperDoubleUrlEncode, TamperRandomCase,
    TamperMySqlVersionComment, TamperMySql50000Comment, TamperInlineComment,
    TamperDoubleKeyword, TamperHtmlEncode, TamperUnicodeEscape, TamperNullByte,
    TamperEqualToLike, TamperLogicalOperators, TamperHexEncode, TamperSleepToBenchmark,
    TamperUnionSelectNospace, TamperEquivalentFunctions, TamperVersionComment,
    TamperCaseCommentMix, TamperScientificNotation, TamperHexKeyword,
    TamperStringConcatBypass, TamperBetweenOperator, TamperOdbcEscape,
    TamperBacktickIdentifiers, TamperKeywordNewlineSplit, TamperHppMarker,
    // new 22
    TamperCharEncode, TamperCharDoubleEncode, TamperCharUnicodeEncode,
    TamperApostropheMask, TamperApostropheNullEncode, TamperUnMagicQuotes,
    TamperBase64Encode, TamperOverlongUtf8, TamperRandomComments, TamperMultipleSpaces,
    TamperGreatest, TamperCommalessLimit, TamperCommalessMid, TamperConcat2ConcatWs,
    TamperIfNull2IfIsNull, TamperModSecurityZeroVersioned, TamperVersionedKeywords,
    TamperUnionAllToUnion, TamperPlus2Concat, TamperPlus2FnConcat,
    TamperBlueCoat, TamperSpPassword, TamperSymbolicLogical,
    // additional 16
    TamperPercentage, TamperSpace2Plus, TamperSpace2Dash, TamperSpace2Hash,
    TamperSpace2MssqlHash, TamperSpace2RandomBlank, TamperSpace2MysqlBlank,
    TamperLowercase, TamperHalfVersionedMoreKeywords, TamperSleep2GetLock,
    TamperLeast, TamperNonRecursiveReplacement, TamperInformationSchemaComment,
    TamperMisUnion, TamperEscapeQuotes, TamperIfNull2CaseWhenIsNull,
};
...
impl TamperChain {
    pub fn new() -> Self { Self { scripts: Vec::new() } }
    ...
    pub fn apply(&self, payload: &str) -> String {
        self.scripts.iter().fold(payload.to_string(), |acc, t| t.tamper(&acc))
    }
    ...
    pub fn from_names(names: &[&str]) -> Self {
        ...
        for &name in names {
            match name {
                ...
                "space_to_whitespace_mix"   => chain.scripts.push(Box::new(TamperSpaceToWhitespaceMix)),
                ...
                "modsecurityzeroversioned"  => chain.scripts.push(Box::new(TamperModSecurityZeroVersioned)),
                ...
                "halfversionedmorekeywords"   => chain.scripts.push(Box::new(TamperHalfVersionedMoreKeywords)),
                "sleep2getlock"               => chain.scripts.push(Box::new(TamperSleep2GetLock)),
                "least"                       => chain.scripts.push(Box::new(TamperLeast)),
                "nonrecursivereplacement"     => chain.scripts.push(Box::new(TamperNonRecursiveReplacement)),
                "informationschemacomment"    => chain.scripts.push(Box::new(TamperInformationSchemaComment)),
                "misunion"                    => chain.scripts.push(Box::new(TamperMisUnion)),
                "escapequotes"                => chain.scripts.push(Box::new(TamperEscapeQuotes)),
                "ifnull2casewhenisnull"       => chain.scripts.push(Box::new(TamperIfNull2CaseWhenIsNull)),
                _ => {}
            }
        }
        chain
    ...
    pub fn available_names() -> &'static [&'static str] {
        &[
            // encoding
            "urlencode", "double_urlencode", "charencode", "chardoubleencode",
            "charunicodeencode", "unicode_escape", "base64encode", "overlongutf8",
            "hex_encode", "html_encode",
            // quote bypass
            "apostrophemask", "apostrophenullencode", "unmagicquotes",
            // space substitution
            "space_to_comment", "space_to_tab", "space_to_newline", "space_to_whitespace_mix",
            "multiplespaces", "bluecoat",
            // keyword obfuscation
            "randomcase", "randomcomments", "inline_comment", "case_comment_mix",
            "double_keyword", "keyword_newline_split", "hex_keyword",
            // MySQL comment tricks
            "mysql_version_comment", "mysql50000comment", "version_comment",
            "versionedkeywords", "modsecurityzeroversioned",
            // operator/function substitution
            "equal_to_like", "greatest", "between_operator", "logical_operators",
            "symboliclogical", "equiv_functions", "ifnull2ifisnull",
            // MySQL syntax variants
            "commalesslimit", "commalessmid", "concat2concatws", "unionalltounion",
            "union_select_nospace", "sleep_to_benchmark", "plus2concat",
            // ODBC / multi-backend
            "odbc_escape", "plus2fnconcat",
            // MSSQL
            "sp_password",
            // misc
            "null_byte", "scientific_notation", "string_concat_bypass",
            "backtick_identifiers", "hpp_marker",
            // additional 16
            "percentage", "space2plus", "space2dash", "space2hash",
            "space2mssqlhash", "space2randomblank", "space2mysqlblank",
            "lowercase", "halfversionedmorekeywords", "sleep2getlock",
            "least", "nonrecursivereplacement", "informationschemacomment",
            "misunion", "escapequotes", "ifnull2casewhenisnull",
        ]
    }
...
impl TamperScript for TamperChain {
    ...
    fn description(&self) -> &'static str { "Composed tamper chain (multiple scripts applied left-to-right)" }
    fn tamper(&self, payload: &str) -> String { self.apply(payload) }
}

...
impl AiAdvisor {
    ...
    ///
    /// Wraps the entire backend call in a hard timeout (`timeout_secs`) so a
    /// slow Ollama model or unresponsive remote API cannot stall the scan.
    pub async fn suggest(&self, ctx: &TargetContext) -> Vec<AiSuggestedPayload> {
        ...
        match tokio::time::timeout(deadline, self.call_backend(ctx)).await {
            Ok(Ok(payloads)) => {
                debug!(
            ...
            Ok(Err(e)) => {
                warn!("AI advisor error (falling back to static payloads): {}", e);
                vec![]
            }
            Err(_elapsed) => {
                warn!(
                    "AI advisor timed out after {}s (falling back to static payloads)",
...
/// Sanitize a user-controlled string before embedding it in the AI prompt.
///
...
///
/// Uses bracket-depth tracking instead of `rfind(']')` so payloads that
/// contain `]` characters (common in SQL: `SUBSTRING(x,1,1)]`) don't cause
/// the parser to cut the array short.
fn parse_payload_json(raw: &str) -> Result<Vec<AiSuggestedPayload>> {
    let trimmed = raw.trim();

    // Fast path: the whole string is already a valid JSON array.
    ...
    // Walk forward from `start` tracking bracket depth and string state
    // so we land on the matching `]`, not an inner one inside a payload string.
    ...
    let end = end.ok_or_else(|| anyhow!("Unclosed JSON array in AI response"))?;

...
impl MarkdownReport {
    ...
    pub fn from_findings(findings: &[crate::sqx::models::SqliTestResult], target_url: &str) -> String {
        ...
        md.push_str("## Remediation\n\n");
        md.push_str("- Use parameterized queries / prepared statements\n");
        md.push_str("- Implement input validation (allowlist approach)\n");
        md.push_str("- Apply principle of least privilege to database accounts\n");
        md.push_str("- Deploy a WAF as defense-in-depth\n");

        md
    }

    /// Generate a Markdown report from a batch scan.
    pub fn from_batch(batch: &[(String, Vec<crate::sqx::models::SqliTestResult>)]) -> String {
        ...
        md.push_str(&format!("**Vulnerable targets:** {}\n", vuln_targets));
        md.push_str(&format!("**Total findings:** {}\n\n", total));

        if total == 0 {
            md.push_str("No SQL injection vulnerabilities detected in any target.\n");
            return md;
        }
        ...
        for (url, findings) in batch {
            ...
            for (i, f) in findings.iter().enumerate() {
                ...
                md.push_str(&format!("- **Confidence:** {:.0}%\n", f.confidence * 100.0));
                if let Some(ref dbms) = f.dbms_hint {
                    md.push_str(&format!("- **DBMS:** {}\n", dbms));
                }
                md.push_str(&format!("- **Evidence:** {}\n", f.evidence));
                md.push_str(&format!("- **Payload:** `{}`\n\n", f.payload));
